...
$form.onsubmit = (e) => {
    ...
    xhr.onreadystatechange = () => {
        ...
        const [title, content, onclick] = {
            ...
            success: ['회원가입', '회원가입해 주셔서 감사합니다. 입력하신 이메일로 계정을 인증할 수 있는 링크를 전송하였습니다.<br><br> 계정 인증 후 로그인할 수 있으며, 해당 링크는 24시간 동안만 유효하니 유의해 주세요.', ($dialog) => {
                Dialog.hide($dialog)
                location.href = '/login';
            }],
            social_success: ['회원가입', '회원가입해 주셔서 감사합니다.', ($dialog) => {
                Dialog.hide($dialog)
                location.href = '/login';
            }],
